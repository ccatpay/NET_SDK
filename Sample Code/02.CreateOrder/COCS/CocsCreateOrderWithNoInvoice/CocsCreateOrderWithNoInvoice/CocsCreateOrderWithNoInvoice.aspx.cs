        ...
        protected void btSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (AllCreateOrder NoInvoice = new AllCreateOrder())
                {
                    ...
                    string limitProductId = (!String.IsNullOrWhiteSpace(tbLimitProductId.Text))
                                           ? tbLimitProductId.Text
                                           : "esun.normal|esun.m3";
                    ...
                    CocsOrderModel order = new CocsOrderModel()
                    {
                        ...
                        /* 訂單參數，必填 */
                        , Command = "CocsOrderAppend"                               // 交易代碼 (COCS代收代付 固定帶入 CocsOrderAppend)
                        ...
                        , LimitProductId = limitProductId                           // 限定產品別
                    ...
                    if (enErrors.Count <= 0)
                    {
                        ...
                        if (String.IsNullOrWhiteSpace(token.Error))
                        {
                            ...
                            ReturnCocsOrder returnOrder = NoInvoice.CreateOrder<ReturnCocsOrder, CocsOrderModel, CocsInvoiceModel>(order, invoice);
