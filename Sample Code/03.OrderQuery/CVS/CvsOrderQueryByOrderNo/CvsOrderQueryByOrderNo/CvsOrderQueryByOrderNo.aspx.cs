...
{
    public partial class CvsOrderQueryByOrderNo : Page
    {
        private readonly UtilityProcess _utilityProcess;

        public CvsOrderQueryByOrderNo()
        {
        ...
        protected void btSubmit_Click(object sender, EventArgs e)
        {
            ...
            List<ReturnCvsOrder> returnOrderList = new List<ReturnCvsOrder>();
            ...
            try
            {
                using (AllOrderQuery AllQuery = new AllOrderQuery())
                {
                    ...
                    OrderQueryModel order = new OrderQueryModel()
                    {
                        ...
                        /* Order 參數 */
                        , Command = "CvsOrderQuery"                                     // 交易代碼 (CVS 訂單單筆查詢 固定帶入 CvsOrderQuery)
                    ...
                    if (enErrors.Count <= 0)
                    {
                        ...
                        if (String.IsNullOrWhiteSpace(token.Error))
                        {
                            ...
                            ReturnCvsOrder returnOrder = AllQuery.OrderQuery<ReturnCvsOrder>(order);
