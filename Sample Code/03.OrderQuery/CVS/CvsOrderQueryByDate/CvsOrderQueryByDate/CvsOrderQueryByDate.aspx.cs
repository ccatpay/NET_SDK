...
{
    public partial class CvsOrderQueryByDate : Page
    { 
        private readonly UtilityProcess _utilityProcess;

        public CvsOrderQueryByDate()
        {
        ...
        protected void btSubmit_Click(object sender, EventArgs e)
        {
            ...
            ReturnCvsOrderList returnOrderList = new ReturnCvsOrderList();
            ...
            try
            {
                using (AllOrderQuery AllQuery = new AllOrderQuery())
                {
                    ...
                    OrderQueryModel order = new OrderQueryModel()
                    {
                        ...
                        /* Order 參數 */
                        , Command = "CvsOrderListQuery"                               // 交易代碼 (CVS 訂單批次查詢 固定帶入 CvsOrderListQuery)
                    ...
                    if (enErrors.Count <= 0)
                    {
                        ...
                        if (String.IsNullOrWhiteSpace(token.Error))
                        {
                            ...
                            returnOrderList = AllQuery.OrderListQuery<ReturnCvsOrderList>(order);
            ...
            {
                List<ReturnCvsOrder> orderList = new List<ReturnCvsOrder>();
        ...
        protected void BindGridList(List<ReturnCvsOrder> orderList)
        {
