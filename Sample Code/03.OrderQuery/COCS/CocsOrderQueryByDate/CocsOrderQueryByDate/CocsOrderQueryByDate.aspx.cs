...
{
    public partial class CocsOrderQueryByDate : Page
    { 
        private readonly UtilityProcess _utilityProcess;

        public CocsOrderQueryByDate()
        {
        ...
        protected void btSubmit_Click(object sender, EventArgs e)
        {
            ...
            ReturnCocsOrderList returnOrderList = new ReturnCocsOrderList();
            ...
            try
            {
                using (AllOrderQuery AllQuery = new AllOrderQuery())
                {
                    ...
                    OrderQueryModel order = new OrderQueryModel()
                    {
                        ...
                        /* Order 參數 */
                        , Command = "CocsOrderListQuery"                                // 交易代碼 (CVS 訂單批次查詢 固定帶入 CocsOrderListQuery)
                    ...
                    if (enErrors.Count <= 0)
                    {
                        ...
                        if (String.IsNullOrWhiteSpace(token.Error))
                        {
                            ...
                            returnOrderList = AllQuery.OrderListQuery<ReturnCocsOrderList>(order);
            ...
            {
                List<ReturnCocsOrder> orderList = new List<ReturnCocsOrder>();
        ...
        protected void BindGridList(List<ReturnCocsOrder> orderList)
        {
