            ...
            try
            {
                using (CreditCard creditCard = new CreditCard())
                {
                    ...
                    CocsOrderModel cancel = new CocsOrderModel()
                    {
                        ...
                        /* Credit Card 參數 */
                        , Command = "CocsOrderCancel"                                   // 交易代碼 (COCS 刷卡取消授權 固定帶入 CocsOrderCancel)
                    ...
                    if (enErrors.Count <= 0)
                    {
                        ...
                        if (String.IsNullOrWhiteSpace(token.Error))
                        {
                            ...
                            ReturnBasic rtn = creditCard.CocsOrderCancel(cancel);
