            ...
            try
            {
                using (MobilePayment creditCard = new MobilePayment())
                {
                    ...
                    DphOrderModel cancel = new DphOrderModel()
                    {
                        ...
                        /* Credit Card 參數 */
                        , Command = "DphOrderRefund"                                    // 交易代碼 (DPH 取消交易 固定帶入 DphOrderRefund)
                    ...
                    if (enErrors.Count <= 0)
                    {
                        ...
                        if (String.IsNullOrWhiteSpace(token.Error))
                        {
                            ...
                            ReturnOrder rtn = creditCard.DphOrderRefund(cancel);
