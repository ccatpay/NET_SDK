...
{
    public partial class RequestDphCash : Page
    {
        private readonly UtilityProcess _utilityProcess;

        public RequestDphCash()
        {
            _utilityProcess = new UtilityProcess();
        }

        /// <summary>
        /// DPH 指定請款金額 (僅限 OPEN 錢包)
        /// </summary>
        ...
        protected void btSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (MobilePayment creditCard = new MobilePayment())
                {
                    ...
                    decimal crAmount = Decimal.TryParse(tbCrAmount.Text, out crAmount)
                                      ? crAmount : 0;
                    ...
                    DphOrderModel cancel = new DphOrderModel()
                    {
                        ...
                        /* Credit Card 參數 */
                        , Command = "DphCashRequest"                                    // 交易代碼 (DPH 指定金額請款 固定帶入 DphCashRequest)
                        , CustomerOrderNo = tbCustomerOrderNo.Text                      // 契客訂單號碼
                        , OrderAmount = orderAmount                                     // 訂單/交易金額
                        , CrAmount = crAmount                                           // 指定請款金額
                    ...
                    if (enErrors.Count <= 0)
                    {
                        ...
                        if (String.IsNullOrWhiteSpace(token.Error))
                        {
                            ...
                            ReturnOrder rtn = creditCard.DphCashRequest(cancel);
